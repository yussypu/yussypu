...
[caliper](https://github.com/yussypu/caliper): tick to trade latency lab measured two ways that bracket the path. AF_XDP kernel bypass isolates the userspace reaction at 70 ns across decode, book, decide and encode. Real NIC hardware timestamps over two hosts, an Intel i210 PHC on each side of a vSwitch, give a true wire to wire of 28.6 us at the median, and the gap between the two is the kernel and NIC tax that AF_XDP exists to skip. Every percentile corrected for coordinated omission against a paced grid, hardware tx stamp yield 100 percent over 2.08M real NASDAQ orders.
